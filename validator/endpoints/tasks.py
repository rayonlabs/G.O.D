...
from core.models.payload_models import NewTaskRequestInstructToChat
...
from validator.tasks.synthetic_scheduler import merge_and_upload_chat_datasets
...
TASKS_CREATE_CHAT_FROM_INSTRUCT_ENDPOINT = "/v1/tasks/create_chat_from_instruct"
...
async def create_chat_task_from_instruct_ds(
    request: NewTaskRequestInstructToChat,
    config: Config = Depends(get_config),
) -> NewTaskResponse:
    """Create a new chat task from instruct-style dataset parameters by transforming the dataset format"""
    ...
    try:
        chat_dataset_url = await merge_and_upload_chat_datasets(
            dataset_ids=[request.ds_repo], input_field=request.field_input, output_field=request.field_output
        )
        ...
        logger.info(f"Created chat task from instruct params: {task.task_id}")
        return NewTaskResponse(success=True, task_id=task.task_id, created_at=task.created_at, account_id=task.account_id)

    except Exception as e:
        logger.error(f"Failed to create chat task from instruct params: {e}")
...
def factory_router() -> APIRouter:
    ...
    router.add_api_route(TASKS_CREATE_CHAT_FROM_INSTRUCT_ENDPOINT, create_chat_task_from_instruct_ds, methods=["POST"])
