...
async def _add_new_task_to_network_if_not_enough(
    ...
    current_training_tasks = await get_tasks_with_status(TaskStatus.TRAINING, config.psql_db, tournament_filter="exclude")
    current_preeval_tasks = await get_tasks_with_status(TaskStatus.PREEVALUATION, config.psql_db, tournament_filter="exclude")
    current_delayed_tasks = await get_tasks_with_status(TaskStatus.DELAYED, config.psql_db, include_not_ready_tasks=True, tournament_filter="exclude")
