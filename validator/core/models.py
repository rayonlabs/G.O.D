...
class ColumnPickerResponse(BaseModel):
    field_instruction: str
    field_output: str
    field_input: str | None = None
    ...
    def validate_columns_ok(self, sparse_columns: list[str], non_sparse_columns: list[str]) -> bool:
        ...
        # Only check for duplicates among non-None columns
        columns = [col for col in [self.field_input, self.field_instruction, self.field_output] if col is not None]
