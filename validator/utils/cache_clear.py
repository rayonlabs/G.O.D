...
def remove_cache_models_except(models_to_keep: list[str]):
    ...
    existing_models = {
        ...
        for orig_dir in os.listdir(cache_dir)
    ...
    logger.info(f"Cleaned cache: kept {len(models_to_keep)} models, removed {deleted_count} models.")
...
def get_hf_models_cache_size() -> int:
    """Get the total size of HuggingFace models cache in bytes."""
    cache_dir = os.path.expanduser("~/.cache/huggingface/hub")
    ...
    if os.path.exists(cache_dir):
        for dir_name in os.listdir(cache_dir):
            if dir_name.lower() == model_pattern:
                return get_directory_size(os.path.join(cache_dir, dir_name))
    return 0
...
def manage_models_cache(model_stats: dict[str, dict], max_size: int) -> None:
    ...
    current_size = get_hf_models_cache_size()
    ...
    final_size = get_hf_models_cache_size()
