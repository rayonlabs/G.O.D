...
from validator.core.constants import CHAIN_WEIGHT_DIVISOR
from validator.core.constants import DEFAULT_RECENT_SUBMISSIONS_LIMIT
from validator.core.constants import DPO_TASK_SCORE_WEIGHT
from validator.core.constants import GRPO_TASK_SCORE_WEIGHT
from validator.core.constants import IMAGE_TASK_SCORE_WEIGHT
from validator.core.constants import INSTRUCT_TEXT_TASK_SCORE_WEIGHT
from validator.core.constants import MINER_PERFORMANCE_CACHE_KEY_PREFIX
from validator.core.constants import MINER_PERFORMANCE_CACHE_TTL
from validator.core.constants import ONE_DAY_SCORE_WEIGHT
from validator.core.constants import SEVEN_DAY_SCORE_WEIGHT
from validator.core.constants import THREE_DAY_SCORE_WEIGHT
from validator.core.miner_models import MinerDetailsResponse
from validator.core.miner_models import MinerPerformanceMetrics
from validator.core.miner_models import OnChainIncentive
from validator.core.miner_models import PeriodScore
from validator.core.miner_models import PeriodTotals
from validator.core.miner_models import TaskSourcePerformance
from validator.core.miner_models import TaskSubmissionResult
from validator.core.miner_models import TaskTypeBreakdown
from validator.core.miner_models import TaskTypePerformance
from validator.core.miner_models import WeightingDetails
from validator.core.weight_setting import _get_leaderboard_data
from validator.core.weight_setting import get_miner_performance_breakdown
from validator.evaluation.scoring import calculate_adjusted_task_score
...
async def build_miner_details_response(
    hotkey: str, target_node: Node, all_nodes: List[Node], task_results: list, weighting_details: WeightingDetails
    ...
    for task_type_str, (field_name, task_type_enum) in task_type_map.items():
        if task_type_str in breakdown["task_types"]:
            ...
            performance.organic_performance = TaskSourcePerformance(task_count=type_data["total_organic_tasks"])
            performance.synthetic_performance = TaskSourcePerformance(task_count=type_data["total_synthetic_tasks"])
            ...
            miner_type_tasks = [tr for tr in type_tasks if any(ns.hotkey == hotkey for ns in tr.node_scores)]
...
def get_recent_submissions(
    hotkey: str, task_results: list, limit: int = DEFAULT_RECENT_SUBMISSIONS_LIMIT
            ...
            if h == hotkey:
                ...
                model_size_billions = (
                    ...
                    if getattr(task_result.task, "model_params_count", 0) > 0
                        ...
                        8,
                        int(model_size.group(1)) if (model_size := re.search(r"(\d+)(?=[bB])", task_result.task.model_id)) else 1,
                    )
                )
...
def calculate_performance_metrics(hotkey: str, task_results: list) -> MinerPerformanceMetrics:
    ...
    all_scores = [ns.quality_score for tr in hotkey_tasks for ns in tr.node_scores if ns.hotkey == hotkey]
    ...
    task_distribution = {k: (v / total_tasks * 100) if total_tasks > 0 else 0 for k, v in task_counts.items()}
...
async def cache_all_miner_performance(config: Config) -> None:
    ...
    try:
        ...
        for node in all_nodes:
            try:
                response = await build_miner_details_response(node.hotkey, node, all_nodes, task_results, weighting_details)
