...
async def create_text_tournament_round(
...
) -> tuple[TournamentRound, list[RawTask]]:
    if isinstance(round_data, GroupRound):
        ...
        tasks = await _create_one_of_each_text_task(config, use_big_model=True, save_to_db)
    else:
        num_pairs = len(round_data.pairs)
        logger.info(f"Creating text tournament for {num_pairs} knockout pairs (probability-based)")
        tasks = await _create_probability_based_text_tasks(round_data, config, save_to_db)

    tournament_round = TournamentRound(round_structure=round_data, tasks=[str(task.task_id) for task in tasks], is_final_round=is_final_round)
    return tournament_round, tasks


async def create_image_tournament_round(round_data: Round, config: Config, is_final_round: bool = False, save_to_db: bool = True) -> tuple[TournamentRound, list[RawTask]]:
    ...
    if isinstance(round_data, GroupRound):
        ...
        for i, group in enumerate(round_data.groups):
            ...
            while True:
                try:
                    task = await create_synthetic_image_task(config, image_models, save_to_db)
                    break
...
async def _create_group_text_tasks(round_data: GroupRound, config: Config, is_final_round: bool, save_to_db: bool = True) -> list[RawTask]:
    ...
    for i, group in enumerate(round_data.groups):
        ...
        instruct_task = await create_synthetic_instruct_text_task(config, models, instruct_datasets, save_to_db)
        ...
        dpo_task = await create_synthetic_dpo_task(config, models, dpo_datasets, save_to_db)
        ...
        grpo_task = await create_synthetic_grpo_task(config, models, instruct_datasets, save_to_db)
...
async def _create_one_of_each_text_task(config: Config, use_big_model: bool, save_to_db: bool = True) -> list[RawTask]:
    ...
    instruct_task = await create_synthetic_instruct_text_task(
        config, big_models if use_big_model else small_models, instruct_datasets, save_to_db
    )
    gpu_req = get_tournament_gpu_requirement(instruct_task.task_type, instruct_task.model_params_count)
    logger.info(
    ...
    dpo_task = await create_synthetic_dpo_task(config, small_models, dpo_datasets, save_to_db)
    gpu_req = get_tournament_gpu_requirement(dpo_task.task_type, dpo_task.model_params_count)
    logger.info(f"  DPO: {dpo_task.task_id} - Model: {dpo_task.model_id} - Dataset: {dpo_task.ds} - GPU: {gpu_req}")
    tasks.append(dpo_task)

    grpo_task = await create_synthetic_grpo_task(config, small_models, instruct_datasets, save_to_db)
    gpu_req = get_tournament_gpu_requirement(grpo_task.task_type, grpo_task.model_params_count)
    logger.info(f"  GRPO: {grpo_task.task_id} - Model: {grpo_task.model_id} - Dataset: {grpo_task.ds} - GPU: {gpu_req}")
    tasks.append(grpo_task)
...
async def _create_probability_based_text_tasks(round_data: KnockoutRound, config: Config, save_to_db: bool = True) -> list[RawTask]:
    ...
    for i in range(num_tasks):
        ...
        if rand_val < instruct_prob:
            task = await create_synthetic_instruct_text_task(config, models, instruct_datasets, save_to_db)
            task_type = TaskType.INSTRUCTTEXTTASK
        elif rand_val < (instruct_prob + dpo_prob):
            task = await create_synthetic_dpo_task(config, models, dpo_datasets, save_to_db)
            task_type = TaskType.DPOTASK
        else:
            task = await create_synthetic_grpo_task(config, models, instruct_datasets, save_to_db)
