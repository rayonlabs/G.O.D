...
from core.models.tournament_models import TournamentRoundData, RoundType, RoundStatus, TournamentType
from core.models.utility_models import TaskType, TaskStatus
from validator.core.models import InstructTextRawTask, DpoRawTask, GrpoRawTask, ImageRawTask
from validator.tournament.boss_round_sync import sync_boss_round_tasks_to_general, _copy_task_to_general
...
@pytest.mark.asyncio
async def test_sync_boss_round_tasks_schedules_correctly(
    mock_config, mock_psql_db, sample_boss_round, sample_tournament_tasks
):
    """Test that sync_boss_round_tasks_to_general schedules the correct number of tasks."""
    
    with patch('validator.tournament.boss_round_sync.get_tournament_tasks', 
               return_value=sample_tournament_tasks) as mock_get_tasks:
        with patch('asyncio.create_task') as mock_create_task:
            
            await sync_boss_round_tasks_to_general(
                "tourn_abc123_20250713",
                sample_boss_round,
                mock_psql_db,
                mock_config
            )
...
@pytest.mark.asyncio
async def test_copy_task_to_general_creates_correct_copy(
    mock_config, mock_psql_db, sample_instruct_task
):
    """Test that _copy_task_to_general creates a proper copy with updated fields."""
    
    original_task_id = sample_instruct_task.task_id
    
    with patch('validator.tournament.boss_round_sync.get_task', 
               return_value=sample_instruct_task) as mock_get_task:
        with patch('validator.tournament.boss_round_sync.add_task') as mock_add_task:
            with patch('validator.tournament.boss_round_sync._record_task_sync_link') as mock_record_link:
                ...
                mock_record_link.assert_called_once_with(
                    str(original_task_id), 
                    added_task.task_id, 
                    mock_psql_db
                )
...
@pytest.mark.asyncio
async def test_sync_handles_no_tasks_gracefully(
    mock_config, mock_psql_db, sample_boss_round
):
    """Test that sync handles empty task list gracefully."""
    
    with patch('validator.tournament.boss_round_sync.get_tournament_tasks', 
               return_value=[]) as mock_get_tasks:
        with patch('asyncio.create_task') as mock_create_task:
