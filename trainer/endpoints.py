...
from trainer.utils import get_available_gpu_types
...
async def start_training(req: TrainerProxyRequestImage) -> Response:
    ...
    asyncio.create_task(start_training_task(training_job))
    start_task(req)
    
    return {"message": "Started Training!", "task_id": req.task_id}
...
async def get_task_details(task_id: str) -> TrainerImageTaskLog:
